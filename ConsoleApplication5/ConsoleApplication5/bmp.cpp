...
bool AUX_RGBImageRec::loadFile(const char* Filename)
{
	...
	BMInfo.bmiHeader.biBitCount = 0;				// Get windows to open and load the BMP file and handle the messy decompression if the file is compressed  
													...
													//char* char_array = (char*)Filename;
													//std::string Filename_W = char_array;


													//wchar_t Filename_LPWTRW[50];
													//mbstowcs(wtext, text, strlen(text) + 1);//Plus null
													//LPWSTR ptr = wtext;
	wchar_t pwstrDest[200];
	int nLen = (int)strlen(Filename) + 1;
	mbstowcs(pwstrDest, Filename, nLen);

	HANDLE DIBHandle = LoadImage(0, pwstrDest, IMAGE_BITMAP, 0, 0, LR_DEFAULTCOLOR | LR_CREATEDIBSECTION | LR_LOADFROMFILE);  // use windows to get header info of bitmap - assume no errors in header format 
